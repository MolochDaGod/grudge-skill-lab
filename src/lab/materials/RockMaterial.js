 ...
 *
 * A dedicated material instance is built per mesh (plate, rock, tower) because
 * `MeshStandardMaterial.onBeforeCompile` is shared by reference and a single
 * patch would otherwise compile once and silently leak the wrong uniforms to
 * every mesh that holds it.
 *
 * @param {import('../world/Environment.js').Environment} environment
 *   the scene's environment (calls `registerShadowCasterWithPatch` to compose
 *   the CSM shadow injection with this shader patch)
 * @param {number} [mossAmount]  0..1 master strength on the moss term. Plates
 *   lie flat, so a smaller value stops them from going entirely green.
 */
export function createRockMaterial(environment, mossAmount = 1) {
  ...
  /** Refresh the palette and the hot-seam glow from the editor. */
