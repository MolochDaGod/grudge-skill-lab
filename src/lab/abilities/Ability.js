...
export class Ability {
  ...
  _updateLight(dt, scale) {
    ...
    const hex = cfg.lightColor ?? cfg.colorCore ?? '#ffffff';
    this.lightColor.copy(getColor(hex));
    const shimmer = this.lightShimmer();
    const intensity = (cfg.lightIntensity ?? 12) * scale * shimmer + this.lightBoost;
    const radius = (cfg.lightRadius ?? 10) * (1 + this.lightBoost * 0.02);
    this.ctx.lights.set(
      this.light,
      this.position,
      this.lightColor,
      Number.isFinite(intensity) ? intensity : 0,
      Number.isFinite(radius) ? radius : 8,
      dt
    );
