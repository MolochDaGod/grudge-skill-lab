...
/** Live preview CORS + status poll from grok.com, plus HMR host rewrite. */
function grokPreviewPlugin(): Plugin {
  return {
    name: "grok-preview",
    apply: "serve",
    configureServer(server) {
      server.middlewares.use((req, res, next) => {
        ...
        if (pathOnly === "/__grok-preview/status") {
          res.statusCode = 200;
          res.setHeader("content-type", "application/json; charset=utf-8");
          res.setHeader("Cache-Control", "no-store");
          res.end(JSON.stringify({ ok: true, ready: true, ts: Date.now() }));
          return;
        }
        next();
      });
...
export default defineConfig(({ command, isPreview }) => ({
  ...
  ssr: {
    noExternal: [],
    external: ["three", "@react-three/fiber", "@react-three/drei", "@react-three/rapier", "@dimforge/rapier3d-compat"],
  },
  plugins: [
    grokPreviewPlugin(),
